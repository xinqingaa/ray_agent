...
当前合并规则还有一个边界：**旧计划已经没有未结束步骤时，程序不会合并 Planner 返回的新步骤。** 例如最后一步交回结果后，Planner 即使又提出“补充核对”，这条新安排也不会加入列表；外层再次选择步骤仍取不到待办，随后进入总结。因此，这里的计划更新不是任何时刻都能追加工作的通用调度能力，步骤列表为空也不能独立证明没有遗漏。

检验这个机制可以固定角色的返回内容，分别给出“保留第二步”“移除剩余待办”“最后一步后提出新待办”，再观察程序实际调用了哪一步。这样的本地夹具检验选择与合并规则；真实模型是否作出合理安排，还需另用任务与产物判断。两种证据回答的问题不同，不能互相替代。
...
选择双循环，取决于任务是否需要这些步骤间的交接，而不是循环层数越多越好。

## 两种常见的进度控制方式

第四章的 Agent Loop 回答的是：这一轮还要不要行动。本章的外层循环回答的是：整项任务还剩哪一步。把这两件事分给不同循环，是一种常见的 Harness 设计，但不是唯一的设计。

差别不在会不会写计划，而在**下一步由谁决定**。两边的内层几乎是同一件事——请求模型，有工具调用就执行并回传，没有就收口。分叉在于步骤边界是不是程序里的控制流。

**双循环编排**把编排结构固定下来。外层认「当前步骤」：选择步骤、交给执行器、根据步骤结果更新计划、再选下一步。步骤内容往往仍由 Planner 列出，也可以在更新时改写；程序按步骤状态选择工作，并处理没有下一步时的总结。执行器围绕当前步骤运行 Agent Loop，再把步骤结果交回外层。这是一次调用的职责边界，不是工具权限边界；前面的文件任务已经说明，执行器仍可能提前完成后续工作。这就是 RayAgent 现在的主路径：Planner 提出安排，流程程序按状态推进，执行器选择具体行动。

**单循环 + 计划工具**只固定那一层 Agent Loop。在这里比较的结构中，程序不按计划清单维护步骤指针，也不在「当前步骤结束」时单独交接。模型每一轮提出工具调用，程序按策略处理，再把结果交回模型。若需要让人看见安排，计划可以做成同循环里的一个工具，程序保存并展示清单，但不拿它当调度器。下一步做什么、要不要改计划，可以由模型带着新观察再提出；能否执行、能否继续，仍由程序控制。

Codex 的计划更新可以帮助理解后一种用法，但不要据此把整个产品概括成一个没有其他控制的循环。其官方接口同时提供计划更新事件、运行中补充输入、取消与审批。这里比较的是**计划是否直接参与步骤调度**，不是声称某个产品只有一层循环，也不是推断其设计者的全部动机。

![双循环由编排器推进步骤，单循环把进度留在同一条 Agent Loop 里](assets/07-progress-control.svg)

图的左侧把工具反馈循环放在执行器中，步骤结果交回外层后才更新安排、再选步骤；右侧把计划工具放在同一条工具反馈循环中，没有另一次步骤交接。蓝色表示步骤交接，绿色表示工具反馈循环。图只展开正常推进路径，收口条件写在各自下方。

可以对照理解：

| | 双循环编排 | 单循环 |
|---|---|---|
| 行动与调度 | 程序选择步骤，执行器提出步骤内行动 | 模型提出下一次行动，程序处理并决定是否继续 |
| 顺序 | 按更新后的计划取步骤；更新仍可能移除待办 | 清单表达安排，不直接强制行动顺序 |
| 步骤边界 | 有明确交接点；仅传入步骤说明不限制工具范围 | 没有独立的步骤交接，清单可提供提醒 |
| 改计划 | 本例在步骤返回后请求 Planner 更新 | 模型调用计划工具更新清单 |
| 实现 | 维护步骤推进与工具反馈两种收口条件 | 计划工具与其他工具共享分发和反馈过程 |
| 失败 | 可设计步骤级转移；RayAgent 的 `FAILED` 当前中止本轮 | 可将错误回传模型，也可由程序直接中止 |

没有绝对好坏。双循环便于在步骤之间设置审计或交接点；单循环便于在同一反馈过程中调整行动，例如写代码时读着读着发现应先改测试。RayAgent 展示了前一种组织方式的效果：计划参与程序调度。这些特点提供了选型依据，但不能只凭结构断言哪一种更少跑偏、更省成本，需要在相同任务与运行条件下比较。

两者也不互斥。单循环系统仍可在同循环里加入更硬的任务约束，或为隔离上下文再派生子循环；双循环的内层，本身就是熟悉的 Agent Loop。比较的是控制权放在哪，不是循环层数本身。

关键区别是：**双循环把步骤结果交回外层调度；单循环中的计划工具把安排留在同一反馈过程。两者的运行边界都由程序负责。**

至于用户中途补充要求、执行超时或需要暂停时，谁来改变推进过程，是下一章的任务。
