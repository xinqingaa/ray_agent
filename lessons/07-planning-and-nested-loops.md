...
**双循环编排**把编排结构固定下来。外层认「当前步骤」：选择步骤、交给执行器、根据步骤结果更新计划、再选下一步。步骤内容往往仍由 Planner 列出，也可以在更新时改写；列完之后，谁先做、何时算做完、没有下一步时是否总结，主要由流程程序保证。执行器被关在当前步骤里，用 Agent Loop 把这一步做完，再把步骤结果交回外层。这就是 RayAgent 现在的主路径：Planner 和流程程序管进度，执行器管干活。

**单循环**只固定那一层 Agent Loop。程序不维护步骤指针，也不在「当前步骤结束」时单独交接。模型每一轮自己决定调用哪些工具；同一轮里的多次调用，只是这一轮内部按顺序（或并行）执行，不是外层的下一步。若需要让人看见安排，计划可以做成同循环里的一个工具，程序保存并展示清单，但不拿它当调度器。下一步做什么、要不要改计划、失败后换路还是重试，都由模型带着新观察再提一次。这种搭法的代表是 Codex：循环怎么转、进度怎么走，都交给 Agent 在轮次之间自己决定。

![双循环由编排器推进步骤，单循环把进度留在同一条 Agent Loop 里](assets/07-progress-control.svg)

图的上行展示步骤之间的交接：编排器选择步骤，执行器交回步骤结果，再由编排器推进。下行展示同一条 Agent Loop 中的连续决策：程序回传工具结果，Agent 根据新观察选择下一次行动。图省略循环回线，只突出这两种交接粒度。

可以对照理解：

| | 双循环编排 | 单循环 |
|---|---|---|
| 控制权 | 编排器管进度，执行器管当前步骤里的行动 | 循环和进度都由 Agent 下一轮的选择决定 |
| 顺序 | 外层不容易跳步、丢步 | 灵活，模型可改写整份安排 |
| 跑偏 | 内层被关在当前步骤里 | 靠清单提醒；长对话仍可能偏 |
| 改计划 | 中途调整要走外层状态机 | 再调用一次计划工具即可重写 |
| 实现 | 两套生命周期、两套收口条件 | 循环始终一个，计划与其他能力挂在同一套分发上 |
| 失败 | 可按「本步失败 → 重试 / 跳过 / 重规划」做转移 | 错误作为普通工具结果喂回，下一轮由模型消化 |

没有绝对好坏。双循环适合步骤边界清楚、需要在步骤之间审计或交接的任务；单循环适合边界会在执行中变化的工作，例如写代码时读着读着发现应先改测试。RayAgent 选择前者，是为了让「还剩什么」成为程序可读的对象；Codex 选择后者，是为了让同一条循环保持通用，步骤记忆交给模型（以及可选的可见清单）。

两者也不互斥。单循环系统仍可在同循环里加入更硬的任务约束，或为隔离上下文再派生子循环；双循环的内层，本身就是熟悉的 Agent Loop。比较的是控制权放在哪，不是循环层数本身。

一句话收束：**双循环里，Planner 与编排器管进度，Agent（执行器）只负责把当前步骤执行完；单循环以 Codex 为代表，循环如何继续、进度如何编排，都由 Agent 自己决定。**
