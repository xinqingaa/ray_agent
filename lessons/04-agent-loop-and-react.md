...
因此，上一章的伪代码还缺最后一步：执行之后，不要换一套规则。下面的教学伪代码包含通用控制条件；本章主实验实现迭代上限，没有实现完整的取消与等待机制：

```text
把用户请求追加到 messages
重复
  检查取消、执行上限等控制条件；不满足时停止或等待
  请求模型
  把这条 assistant message 追加到 messages
  如果这一轮是行动请求：
    检查调用是否允许执行，执行后取得成功或错误观察
...
一次模型响应结束，只说明本次生成告一段落；循环结束，说明程序暂时不再推进；任务完成，还需要检查用户目标是否满足。这三个判断可能发生在不同时间。

在文件任务里，模型直接回复“已经完成”而没有读取证据，实验仍可能因为没有 `tool_calls` 而退出。这个停止条件实现简单，却没有独立检查文件内容。若要求可靠确认，就需要根据任务设置结果检查，例如核对文件实际存在且内容符合要求；不能仅靠最终文本的语气判断成功。

控制边界还决定系统如何停下。迭代上限限制循环次数，不能单独限制某一次工具执行的耗时；超时、取消和等待输入分别处理不同情形。继续重试可以给失败恢复的机会，也会消耗资源，重复写入等操作还需要考虑副作用。本章先理解这些条件为何属于程序的控制职责，具体策略在后续展开。
