...
| [09 · 状态与持久化](09-state-and-persistence.md) | 执行中的信息分别归谁，能够保存多久？ | RayAgent：状态模型、存储与历史 | 如何用事件理解、展示和诊断一次执行？ |
| [10 · 事件与可观察性](10-events-and-streaming.md) | 如何用事件理解、展示和诊断一次执行？ | RayAgent；按需对照异步 labs | 工具在哪里执行，环境由谁管理？ |
...
第 01 章回答为什么需要 Harness，建立职责直觉；第 06 章回答已学机制如何在一条真实任务中交接，建立系统结构；第 07 章深入 Plan + ReAct 的两层交接、终止条件和设计代价。第一章只概述规划选择，不提前展开嵌套循环。

第 06 章以“在沙箱写入 hello.txt，再读取并总结”建立共同观察，沿任务解释 Context、模型调用、执行控制、工具反馈、状态、事件与产物。服务条件链接 [应用指南](../ray_agent/README.md)，运行操作服务于机制观察，不写成部署或界面教程。本章不承担后续各机制的完整讲解。
...
第 08 章覆盖运行控制、超时、预算与人工介入；第 09 章区分保存、回放和恢复；第 10 章兼顾事件传递、追踪与诊断；第 11 章解释环境生命周期和访问边界；第 16 章以任务样本、检查依据和失败场景组织验证与评估；第 17 章研究跨会话衔接、项目状态和产物的持续使用。通用策略可以超出现有实现，但必须标明能力差距，不把待设计的机制写成产品事实。
