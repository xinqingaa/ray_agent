...
![文件读取后的一份 Context 快照：指令、目标、历史行动、工具观察与工具说明](assets/05-request-working-set.svg)

图展示文件读取后的一份输入快照，每类信息都配有具体内容。这里按用途分组，不规定它们必须对应独立消息或某个框架组件。外部文件通过读取、内容提取等步骤，才能成为可用材料；只提供路径时，模型获得的是定位线索，尚未获得正文。
...
图以用户明确要求“本项目报告只用正式版”为例：任务 A 保存要求，任务 B 按项目找回，并把它与新目标一起放入 Context。上方是写入路径，下方是读取路径；两条路径不必在同一次任务中发生。这是通用设计示意，不是 RayAgent 的现有组件图。
...
以“正式版日期是否确定”为检查问题，可以把原文和处理后的材料交给同一套判断标准。这里先做材料核对，再考虑模型是否能正确使用它：

| 待核对内容 | 原文与合格摘要应保留什么 | 只保留定位线索时怎么办 |
|---|---|---|
| 日期 | 6 月 20 日是有条件的安排 | 读取交付安排后确认 |
| 前提 | 必须评审通过；当前没有通过依据 | 同时寻找评审状态，不能只读取日期 |
| 结论来源 | 资料 A 的交付安排，区分事实与待查项 | 保留可访问的位置及必要版本线索 |

若压缩后的材料只能支持“6 月 20 日发布”，说明处理过程丢失了条件，即使 token 更少也不能算改进。若线索指向的文件已删除、权限变化或正文更新，则应把原判断标为待核实，寻找有效来源或反馈缺口，不能用旧摘要补成确定结论。重新读到新版，也不能自动当成当时原文的副本。

真正比较压缩效果时，应保持任务、原始资料和模型设置一致，分别检查条件遗漏、事实误读、能否继续任务、额外读取次数与实际可得的用量。材料表可以静态核对；模型是否稳定答对需要重复运行。这里提供的是教学判断方法，不是本章已经完成的模型评估。

### 项目指令和技能也要选择与加载

需要处理代码项目时，输入还可能包括项目规则与某类任务的方法。例如项目规则约定测试入口，发布技能描述发布前检查；两者都可能很长，未必适合在每轮调用中完整加载。

一种通用设计是先提供简短目录：材料叫什么、适用什么任务、到哪里读取；任务需要时再加载正文。这与前面的按需读取有共同点：减少常驻内容，代价是可能漏选、选错或增加读取。目录不是正文，技能被列出也不表示模型已经读过并执行了全部要求。持续有效的任务约束还需要在压缩后保留，不能只留下一个已经失效的链接。

加载时还应区分材料的信任来源与适用范围。项目文件、外部网页、用户授权不是同一种依据，外部材料中的命令式文字不能因为被读入就获得更高权限。此处说明可选的上下文组织方式；RayAgent 当前主路径使用预设角色提示与消息记忆，尚未在这条路径实现项目指令或技能目录的按需加载系统。
...
产品也有一个名为 `compact()` 的操作，位于 `ray_agent/api/app/domain/models/memory.py`。当前实现会把 `browser_view`、`browser_navigate` 对应工具消息的正文替换为 `(removed)`，并移除消息中的 `reasoning_content` 字段。正常路径中，步骤返回后进入记忆清理，再更新计划；流程遇到步骤状态为 `FAILED` 时会先中止，不走清理分支。这不是检查模型报告的 `success` 是否为真，两种判断的区别将在第七章展开。
...
[上一章：Agent Loop 与 ReAct](04-agent-loop-and-react.md) · [课程目录](README.md) · [下一章：从 Agent Loop 到完整 Harness](06-from-agent-loop-to-harness.md)
