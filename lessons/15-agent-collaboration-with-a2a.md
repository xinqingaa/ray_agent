...
协作时要连续解决几层问题。协议能约定其中一部分，其余仍由调用方的 Harness 决定。后文按这张表展开：

| 协作时要解决什么 | 协议能约定什么 | 仍要由 Harness 决定什么 | 后文 |
|---|---|---|---|
| 谁组织委派、谁组织执行 | 双方交换消息、状态和成果 | 选择对端、验收本地任务 | 本节 |
| 怎样发现对端并发出委托 | 卡片、接口与 Message | 委托写什么、材料是否够用 | 第二节 |
| 怎样跟踪同一项远程工作 | Task 状态、产物与观察方式 | 何时等待、如何解释中断和终态 | 第三节 |
| 怎样进入本地工具循环 | 不规定任务循环 | 配置、委派工具、结果消费 | 第四节 |
| 接通后怎样控制等待、取消和验收 | 取消与幂等能力 | 超时策略、续接、授权与成果检查 | 第五节 |

读表时先抓住一个区分：**远程完成不等于本地用户任务完成。** 协议提供协作对象，Harness 仍要组织委托、跟踪和验收。下面先看清双方职责，再对照 MCP。
...
这张表列出协议对象；第一节总表给出后文路径。接下来先走通最简单的路径：找到一个服务，发出消息，得到直接回复。
...
取得 Task 后，可以用轮询、流式更新或推送观察进展。它们都不保证远程业务成功；状态更新流也不等于模型 token 流，更不能还原远程全部工具轨迹。[流式与异步交互说明](https://a2a-protocol.org/latest/topics/streaming-and-async/)
...
本章本地实验和协议测试已运行；真实模型与完整 Web 委派任务仍 `unverified`。条件与覆盖范围见 [A2A 实验](../labs/a2a/README.md)与 [API 指南](../ray_agent/api/README.md#mcpa2a)。
