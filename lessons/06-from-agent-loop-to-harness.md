...
本章的页面、事件与文件观察来自一次本机 Compose 运行。运行入口与配置见[应用指南](../ray_agent/README.md)。阅读时不必先启动服务；想复现时，可以在 Web 新建任务并提交下面的请求。模型可能选择不同的步骤与调用顺序，我们对照的是交接机制。
...
这是本次真实界面截图。左侧是总结与附件入口，右侧是最终附件预览。它展示的是交付结果；文件怎样到达这里，还需要向前追踪。
