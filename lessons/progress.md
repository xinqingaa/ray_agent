...
第 01–04 章正文定稿；第 05 章已按通用概念与机制重写，三张配图已重新构图，分别采用具体输入快照、跨任务记忆读写和材料表示对照，风格为 Fireworks Style 1，待内容审阅。正文深度与配图文字密度原则已沉淀到通用写作 skill，课程默认风格已改为 Style 1。foundations 聊天脚本已统一为 `LLM_*`，可与产品 `.env` 使用同一组。其余章节仍为骨架。
...
| [05 · 上下文与记忆](05-context-and-memory.md) | 重写完成，待审阅 | 通用概念与原始资料、产品源码静态核对；`5_1` 实际运行，5 项本地夹具测试通过；3 张 Fireworks Style 1 SVG（1600×900）重新构图后，结构、箭头、几何与构图检查及 Chromium 渲染目视检查通过；真实 token 与 `4_2` 分词 unverified | 审阅正文深度与视觉表达；真实 token 验证仍待完整词表或模型 |
