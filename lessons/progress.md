...
第 01 章正文完成；第 02 章正文定稿，以 OpenAI 兼容 Chat Completions 为基线，DeepSeek 仅为实验落点。其余章节仍为骨架。

下一步：制作第 03 章，从同一条 message 的 `content` / `tool_calls` 分流讲到一次工具执行。第 02 章待模型密钥可用后补真实服务观察；第 06、07 章共同任务的产品复跑仍待开展。
...
| [03 · 工具与行动](03-tools-and-actions.md) | 骨架 | 未进行本章运行验证 | 从 `content` / `tool_calls` 分流讲声明、执行与 `role: tool` 回传 |
