...
接入时要连续解决几层问题。协议能约定其中一部分，其余仍由应用围绕任务决定。后文按这张表展开：

| 接入时要解决什么 | 协议能约定什么 | 仍要由 Harness 决定什么 | 后文 |
|---|---|---|---|
| 谁描述能力、谁把它组织进任务 | 角色分工与消息格式 | 如何使用模型、筛选哪些工具 | 本节 |
| 一次调用如何发现、表达和返回 | 发现、列表、调用与结果形态 | 如何映射模型选择、如何解释失败 | 第二节 |
| 消息走哪条通道、能力在哪执行 | 传输方式 | 谁启动服务、认证与清理 | 第三节 |
| 发现到的工具如何进入已有循环 | 不规定任务循环 | 配置、别名、路由和结果消费 | 第四节 |
| 接通后如何控制失败与副作用 | 取消信号与部分错误形态 | 连接寿命、超时、重试与授权 | 第五节 |

读表时先抓住一个区分：**协议统一的是发现和调用形式，任务循环、工具选择和副作用控制仍由 Harness 承担。** 本章主线是 Tools；Resources 与 Prompts 放到第二节走完一次调用之后。下面先看清参与者，再判断这套约定适合哪些情况。
...
服务运行后，客户端需要知道双方能否使用同一版协议，以及服务支持哪些能力。协议版本约定消息和行为，SDK 版本标识开发库的发行版，二者不是同一个编号。

本章每次请求携带版本与能力元数据，不再用旧版 `initialize` / `initialized` 建立协议级会话。客户端通过 `server/discover` 查询服务支持的版本、能力与自报身份。Server 必须实现这个方法；客户端也可以在已知条件下直接发请求，再处理不兼容错误。[发现规范](https://modelcontextprotocol.io/specification/2026-07-28/server/discover)

发现回答的是“双方可以使用哪些约定”，不是“具体有哪些工具”。`capabilities.tools` 不等于已经拿到了 `add` 的参数定义；服务自报的名称与版本主要用于识别，不能据此判断身份可信。

RayAgent 与本地实验把 Client 固定为 `2026-07-28`，显式发现后检查对端支持列表，不降级到旧版。只支持一个版本更容易验证，也会拒绝不兼容服务。网上的 `initialize()` 示例可能属于另一版协议，不能直接拼进当前流程。
...
走完一次 `add` 调用后，可以再看协议里另外两类能力。资料服务既可以提供搜索工具，也可以提供资料正文和分析模板：
...
Resource 的 URI 是资源标识，不保证是本机路径或可直接下载的网页地址。客户端可以列举并通过 `resources/read` 读取内容；取得之后如何选择、裁剪并交给模型，仍承接第五章和第十二章。[Resources 规范](https://modelcontextprotocol.io/specification/2026-07-28/server/resources)

Prompt 提供可填充的模板，不会自动成为主循环的系统提示，也不会自动获得高于用户目标的权限。协议不强制所有 Host 使用相同界面。[服务端能力说明](https://modelcontextprotocol.io/docs/2026-07-28/learn/server-concepts)

三类能力可以组合，但本章主线仍是 Tools。RayAgent 当前主要集成 Tools，没有完整的 Resource 浏览读取、Prompt 选择应用界面。

发现得到的是能力与参数说明，调用得到的是执行结果。基本客户端实验无需模型就能验证这两件事。接下来需要确定：Client 怎样连到 Server，这些消息通过什么通道传递？
...
设计时可以沿第一节总表那条路径思考：**需要什么能力 → 怎样发现和描述 → 在哪里通信与执行 → 怎样进入模型循环 → 怎样管理资源、失败与权限。** 这些选择落实以后，再分层确认接入是否有效：
...
本章已有本地协议实验与产品适配测试证据；真实模型和完整 Web 任务仍 `unverified`。测试条件与覆盖范围见 [API 指南](../ray_agent/api/README.md#mcpa2a)与[基础实验](../labs/foundations/README.md#mcp-22-可复现基线)。
