...
第8章是运行时怎么离开循环（补充、等待、停止）；第九章是离开那一刻，现场已经裂成几份、各能活多久、新执行者能再接上哪一份。 系统并不会把它们自动对齐成一份。第 8 章的离开方式，可以先看成一张地图。每种情况都要分清留下的是哪一类状态；RayAgent 的选择写在第三列，后文再展开机制和证据。

| 第 8 章留下的问题 | 常见策略 | RayAgent 当前怎么做 |
|---|---|---|
| 等待后新运行器靠什么继续 | 挂起原协程，或结束执行、留下继续条件 | 读取最新计划事件和角色 Memory；工作位置只保存引用，能连上再用 |
| 补充输入为何在事件交接处就能改道 | 以步骤为界，或以事件交接为界 | `yield` 之后才继续；事件和 Memory 不是同一时刻写完 |
| 停止后文件为何还能变 | 记录终态，或连外部副作用一起停 | 会话可以写成 `completed`；工作文件不随取消回滚 |
| 刷新页面 / 请求结束 | 回放历史，或恢复执行 | 能看见写入记录，不等于有人接着跑 |

读表时先抓住两个区分：**历史回放不等于恢复执行**；**程序里的记录不等于工作位置上的文件**。同一项任务没有一份总状态，只有寿命不同的几份现场；第八章每一种中断，问的都是这几份还指不向原来的东西。下面按这些问题分别说明。

## 同一项任务，状态分别归谁

对应第 8 章已经分开的请求、执行和会话：停住之后，它们各自还指向什么。第五章区分了“环境发生了什么、程序记录了什么、模型本次获得了什么”。现在把观察时间延长到对象销毁或进程退出之后：工作位置中的 hello.txt、数据库中的写入记录、下一次请求中的工具结果，可能分别保留，也可能分别丢失。

不要把这件事理解成「文件进存储、计划和记忆进数据库」。核心是按寿命分成三类，系统也不会自动把它们对齐成一份：
...
图按存放位置分组，对应上面的后两类寿命：左侧多是当前进程里的对象，右侧是进程之外还能再访问的位置。箭头由使用方指向被访问的位置，不表示所有内容都经过同一次事务。API 重建执行对象时，可以重新读取数据、访问工作文件，却不能从 task_id 取回已经消失的 Python 调用栈。会话里的环境 ID 和 task_id 一样，只是引用。
...
对应第二行：补充输入为什么能在事件交接处改道。第六章已经说明，同一个工具结果有不同使用者：执行器把它送回模型，运行器补充界面预览并保存事件，文件同步把内容变为可交付副本。本章要进一步问：这些动作是不是同时完成？
...
对应第三行：停止或中断时，会话记录和工作文件为什么可以不在同一位置。继续推演写入 hello.txt 的任务。下面的“未发出”“已生效”是观察者在受控实验中知道的事实；恢复程序只看到残留记录时，未必能判断自己处于哪个窗口。
...
对应第一行：等待退出后，新运行器读什么。正常等待与任意时刻崩溃的区别，在于前者走了明确的交接路径。第八章已经讲过控制过程：`message_ask_user` 经执行器转换为询问和 `WaitEvent`，运行器保存 `waiting` 后退出。现在重点看回复到来时有哪些信息可用。

聊天入口创建新任务和运行器，并按会话里保存的环境 ID 尝试连接原来的工作位置。新 Flow 读取会话，依据 `waiting` 进入执行阶段，从最新 `PlanEvent` 取计划，再选择未结束步骤。Planner 与 ReAct 分别按自己的名称加载 Memory；若末尾是待配对的提问调用，`roll_back()` 将回复包装成该调用的工具结果，供后续模型请求使用。
...
对应第四行：重新打开涉及哪个组件，决定了需要恢复什么。刷新浏览器通常只重建页面状态；API 重启则会丢失内存中的任务对象；执行位置被替换时，即使 API 和数据库都还在，工作文件也可能已不可访问。
...
读完本章，可以对照开头的离开方式表，以及三类寿命检查理解：页面有记录只说明这次组装或会话快照还在，不等于当前进程里还有执行者，也不等于工作文件还在。
...
3. 页面显示步骤运行中，新 Flow 取出的步骤却是 `pending`，是否一定是数据库丢数据？先区分最新 PlanEvent 快照与前端合并 StepEvent 的展示。
