...
这些问题可以先看成一张地图。每种情况都有不止一种策略；RayAgent 的选择写在第三列，后文再展开机制和证据。
...
这张图对应真实 Docker 正常任务：页面可以看到消息、计划、工具调用和最终回复；API 事件记录进一步确认本次运行以 `completed` 和 `done` 收口，并生成了文件附件。
...
因此需要分别观察请求与执行。当前聊天请求没有在结束分支调用任务取消，而显式停止走另一条接口；从源码可以判断二者不是同一个控制动作。一次真实 Docker 任务中，正常 SSE 请求结束后会话记录了 `completed`、`done` 和完整工具事件；这只能说明该次请求的事件收口成功，不能证明所有断连时机下的后台执行和事件回放都可靠。本章仍未完成真实 SSE 断连实验。

## 运行中补充消息，在哪个时刻生效

对应第二行：同一条协程在事件交接处换输入。假设 Agent 正准备读取 `/old.txt`，用户又说：“改为读取 `/new.txt`。”系统可以采用几种策略：等当前工作全部完成后再处理；在交接点切换输入；或者取消当前工作后重新开始。它们对响应速度、已发生副作用和实现复杂度有不同影响，正文需要明确当前策略，不能只说“支持多轮对话”。
...
对应第三行：退出执行，而不是把协程挂起。把文件任务改成“读取那份文件”，但没有提供路径。模型可以询问路径，真正的暂停却需要 Harness 来完成：把问题展示出去，停止进一步行动，记录等待原因，并让后续回复能够找到此前的问题和任务。
...
这里验证的是正常等待交接：确定性夹具中模型响应、仓库与沙箱使用本地替身，计划和事件在每次保存时保留快照。真实 Docker 运行也出现了 `waiting` 和 `wait` 事件；回复 `/home/ubuntu/ch08-normal.txt` 后，新运行器确实继续执行，但该会话自己的动态沙箱中没有这个文件，于是再次等待。这说明恢复会重新进入流程，并不保证之前会话的沙箱内容已经复用。真实模型会不会主动询问、回复内容是否足够、进程中断后记录是否完整，仍需分别验证。尤其是等待退出前后恰好又有消息入队、历史计划缺失等情况，不能凭这条成功路径认定支持任意断点恢复。
