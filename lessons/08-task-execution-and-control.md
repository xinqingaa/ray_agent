...
真实页面的等待状态应与图中的 `waiting` 节点对照阅读。此次运行记录了 `wait` 事件；回复路径后重新启动执行，但动态沙箱没有前一会话的文件，因此再次等待。页面截图用于直观看到状态，事件和日志用于确认交接时刻及沙箱边界。
...
真实停止页面展示的是用户可见的停止结果；API 日志还显示任务取消、注册移除和 `DoneEvent`。本次停止发生在模型规划阶段，所以图中“工具进程”和“沙箱资源”仍不能标记为已清理。
