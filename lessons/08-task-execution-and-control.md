...
第七章已经说明：外层选步骤，内层按工具结果继续决策。循环能推进，还不能回答运行时的控制问题——消息交给谁、中途如何改道、停在哪里、停到哪一层。

这些问题可以先看成一张地图。每种情况都有不止一种策略；RayAgent 的选择写在第三列，后文先讲这个问题要设计什么，再对照当前怎么做。

| 运行时问题 | 常见策略 | RayAgent 当前怎么做 |
|---|---|---|
| 提交之后谁在继续跑 | 同进程协程，或独立工作进程 | 同进程后台协程；按需创建任务和运行器 |
| 运行中又补充一句 | 做完再处理；在交接点切换；取消后重来 | 写入输入流，事件交接后改道；同一条协程，通常重新规划 |
| 缺少路径、需要提问 | 挂起原协程等回复；或结束执行、留下继续条件 | 询问工具立即返回；运行器记下等待后退出；回复时新建运行器 |
| 「问一句」算不算批准 | 把批准绑定到具体动作再放行 | 已有询问和续接，没有执行前审批闸门 |
| 点击停止 | 停调度、停进程、必要时补偿副作用 | 先取消协程并写入终态；不自动杀掉沙箱进程 |
| 跑太久、消耗过多 | 局部次数/超时，或整项任务总预算 | 有迭代、重试和请求超时；没有跨调用的强制总预算 |

读表时先抓住两个区分：**请求、后台执行、会话不是同一个对象**；**补充输入是同一条执行改道，等待是这次执行结束后再组装**。下面按这些问题分别说明。

## 请求交出去了，谁让任务继续

对应表中的第一行：执行单元放在哪里。用户提交“写入 hello，再读取确认”，页面可能持续显示几十秒进展。在这段时间里，有三个不同的对象：请求负责接收输入和返回事件，后台执行负责调用模型与工具，会话负责关联多次交互及其记录。一次会话可以有多次执行；一次请求结束也不直接说明整个目标已完成。
...
RayAgent 采用同进程后台协程。应用协调先查会话与关联任务，需要时准备沙箱、浏览器和运行器，把用户消息写入输入流，再启动运行器。运行器消费输入，进入上一章的规划与执行流程；输出事件经过输出流和请求响应回到页面。

![任务输入、后台执行与事件展示的交接](assets/08-task-handoff.svg)

这张图对应一次真实 Docker 正常任务：页面可以看到消息、计划、工具调用和最终回复；事件记录确认本次运行以 `completed` 和 `done` 收口，并生成了文件附件。

图中上下两部分是同一执行过程的输入与输出方向。消息入队后尝试启动任务；同一个任务实例已经在执行时，不会再启动第二个执行协程。运行器一旦发现输入流为空就会退出，并不是永久驻留、持续监听新消息的工作进程。

当前只有会话仍为 `running`、且本进程还能找到这个任务时，新消息才交给已有执行；否则创建新任务。输入输出流在 Redis，持有协程的注册表仍在 API 进程内。重启后哪些工作还能继续，是第九章的问题。

请求结束和显式停止也不是同一个控制动作：聊天连接收口时不会走停止接口。一次正常任务写完 `completed`、`done` 和工具事件，只说明该次连接和记录收口，不能证明断连后后台一定还在跑。本章仍未做真实 SSE 断连实验。

## 运行中补充消息，在哪个时刻生效

对应表中的第二行：运行中的新输入何时生效。假设 Agent 正准备读取 `/old.txt`，用户又说：“改为读取 `/new.txt`。”系统可以采用几种策略：等当前工作全部完成后再处理；在交接点切换输入；或者取消当前工作后重新开始。它们对响应速度、已发生副作用和实现复杂度有不同影响，正文需要明确当前策略，不能只说“支持多轮对话”。
...
对应表中的第三行：缺少信息时，执行要停在哪里、留下什么才能继续。把文件任务改成“读取那份文件”，但没有提供路径。模型可以询问路径，真正的暂停却需要 Harness 来完成：把问题展示出去，停止进一步行动，记录等待原因，并让后续回复能够找到此前的问题和任务。
...
回复如何与原问题关联？等待发生时，记忆末尾通常还留着尚未配对的提问调用。新流程若认出这个提问，就把用户回复包装成对应调用的工具结果；若末尾是其他未配对调用，则删除末尾消息。它调整的是后续模型请求的消息结构，**不会撤销文件写入或外部操作**。
...
这里验证的是正常等待交接。真实 Docker 运行也出现了 `waiting` 和 `wait` 事件，回复后新运行器继续执行。另一会话用同一路径找不到文件，说明路径依赖执行位置，不证明同会话等待丢失了工作文件；环境归属由第十一章展开。真实模型会不会主动询问、等待前后又有消息入队、历史计划缺失等情况，不能凭这条成功路径认定支持任意断点恢复。

## 等待确认，怎样才构成执行授权

对应表中的第四行：问一句算不算批准。缺少路径是信息不足；目标路径已经存在、是否允许覆盖，是授权问题。两者都可能表现为“问用户一句话”，但继续执行所需的条件不同。补充路径让任务知道操作对象，批准覆盖则明确允许某个动作影响某个对象。
...
对应表中的第五行：停止要确认到哪一层。再把案例改成长操作：进程每隔一小段时间向临时文件追加一行。用户按下停止时，至少有三个需要区分的对象：调用模型或工具的执行协程、执行位置里的操作系统进程，以及已经写入的文件。

停止可以只停调度、把取消传到操作系统进程，必要时再补偿已发生的写入。这三层不必一起完成。即便只取消协程，发出请求也不等于协程已经退出；清理发生在随后的执行机会里。见 [Python 3.12 的任务取消说明](https://docs.python.org/3.12/library/asyncio-task.html#task-cancellation)。

RayAgent 的停止接口查找任务、发出协程取消，随后把会话写成 `completed`，并不等待运行器清理完成。运行器捕获取消后，另外调度收尾协程尝试保存 `DoneEvent` 和终态；原执行协程再清理协议工具资源。独立写库是为避开被取消的执行上下文，不是对写入成功的保证。

![取消请求、终态写入、协程清理与沙箱进程的边界](assets/08-cancel-boundaries.svg)

本次真实停止发生在模型规划阶段：接口返回成功，日志记录了任务取消、注册移除和 `DoneEvent`。图中“工具进程”和“沙箱资源”仍不能标记为已清理。
...
对应表中的第六行：限制消耗之前，先明确限制的是哪一段。取消由外部请求触发；执行预算则让程序在没有用户盯着时也能约束消耗。设计预算前要分清对象：一轮工具反馈、一次请求、一轮用户任务，还是整个会话。范围不清楚，就容易把局部限制误写成全局保证。
...
长操作应只写入可回收的临时目录，并保留独立终止入口；验证之后回收实验进程与目录。实验脚本已经这样处理。真实 Docker 停止实验中，接口返回成功，会话变为 `completed`，日志出现任务取消、注册移除和 `DoneEvent`；但停止发生在模型规划阶段，未进入长 Shell 调用，因此不能证明操作系统进程已被终止。实验后仍能看到动态沙箱容器，说明本次观察也不能证明沙箱资源已经回收。真实 Web 等待、取消的工具中断、重复提交及 SSE 断连仍有缺口；已有第六章成功任务也不能覆盖这些控制路径。

读完本章，可以对照开头的表检查理解：

1. 请求结束不等于任务完成：页面不再推送进展，只说明这一次连接收口；后台协程还在不在，要另查任务实例。
2. 补充输入在事件交接后生效，不是任意时刻：耗时工具正在执行时，新消息可能已经入队，却还不能打断当前动作。
3. 等待是运行器退出后再组装执行，不是协程挂起等待用户：回复会新建任务和运行器，并尽量复用已有沙箱。
4. 页面显示 `completed`，但文件仍在增长，并不自相矛盾：状态写入与进程停止属于不同层。需要追查停止是否传播到真正的执行对象。
5. 用户回复“可以”，并不足以证明覆盖已获准；有迭代上限和 token 统计，也不代表任务已有总预算。前者还要绑定路径、内容、版本和执行前检查，后者必须找到跨调用累计的额度、强制检查位置和触发后的停止处理。
