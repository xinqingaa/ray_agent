...
旧文档里的 `function_call` 是更早的单次字段。当前产品和本章实验使用可包含多项的 `tool_calls`。

还有一种容易看错的情况：有的模型会把 Anthropic 风格的 `tool_use` 写进 `content`。产品在进入执行前，会先把这种内容补成 `tool_calls`，再走同一条路径。本章不展开 Messages API；主路径仍然是：程序读取 `tool_calls`，而不是到正文里猜“它是不是想写文件”。
...
因此，**工具调用的第一个工程问题，是把可执行能力明确写进请求。** 没有声明，模型最多只能在 `content` 里说说“应该写文件”；有了声明，它才可能返回可被程序识别的 `tool_calls`。

声明也只是约束，不是保证。模型可能漏掉必填项、写出不合法的 JSON，或提出一个请求里并不存在的名字。这些都要由程序在执行时处理，不能假定“既然返回了 tool_calls，参数就一定可用”。

## 程序执行，再把结果送回去

接到 `tool_calls` 之后，程序要完成一次交接：按名字找到实现、解析参数、真正执行，再把结果送回模型。可以用几行伪代码表达：
...
如果带有 tool_calls：
  对每一项调用
    ...
    执行，得到结果
    追加一条 role: tool 的消息，带上 tool_call_id 和结果
...
这条路径改变的是职责分工：模型决定“做什么”，程序决定“怎样做、在哪里做、结果是什么”。对文件任务来说，读取返回 `hello`，下一步才有确认内容的依据；读取返回“文件不存在”，下一步的依据就变了。没有执行、没有回传，模型仍然只是在说话。
...
`3_8_Pydantic解析数据.py` 不调用模型。它把一段假想的 `arguments` 字符串校验成对象：合法数据得到 `name` / `age` / `email`，年龄为负或邮箱不合法时失败。这对应执行前必须先解析参数。
...
三种做法都在约束模型的输出形状。只有第一种让程序根据调用去改变外部状态。文件任务需要的是第一种：提出 `write_file`，由程序写入，再把结果送回。后两种有助于看清 `arguments` 和 `content` 都可能携带结构化数据，但不能代替执行。
