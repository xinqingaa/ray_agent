...
前五章建立基础认识，第六章串联完整 Harness，随后展开规划、控制、状态、事件、执行环境与外部能力，最终以可靠性、验证与评估收束。通用策略与项目已实现能力会明确区分。

课程正文已冻结：按目录线性阅读即可；运行命令与实验条件见所属 lab 或服务指南，模块边界见[架构说明](../docs/architecture.md)。
