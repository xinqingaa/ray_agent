...
![课程按认识闭环、完整任务、控制与观察、环境与交付、外部协作、验证展开](assets/course-stages.svg)

读图时每列自上而下，再进入下一列；阶段按主题划分，不绑定章号。具体章节见下方目录。运行命令与实验条件见所属 lab 或服务指南，模块边界见[架构说明](../docs/architecture.md)。
