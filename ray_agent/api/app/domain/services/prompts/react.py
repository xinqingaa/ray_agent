...
- 返回 success JSON 之前，必须已经通过 tool_calls 真正调用了工具。禁止只在 result 里声称已写入、已读取或已搜索。
- attachments 只能填写 write_file 等工具已经创建、且真实存在于沙箱中的路径；没有文件则返回空数组。
...
- attachments 只能列出此前工具确实写入沙箱的路径。没有文件则返回空数组，不要编造路径。
