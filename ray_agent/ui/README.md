...
脚本用项目 TypeScript 转译器加载实际 SSE 解析、事件归一化和时间线模块；不复制实现，也不连接产品服务。确认 LF 逐字节分块（含中文 UTF-8）、非法 JSON 回调、调用阶段合并与新输入后的步骤分组。另将两个当前限制明确打印为 `LIMITATION`：EOF 分派未以空行结束的完整 JSON 尾段，以及 CRLF 恰在 CR/LF 之间分块时丢失事件类型。脚本退出成功表示上述现有行为与限制得到复现，不表示 SSE 标准符合性或全部分块场景通过。
