...
RayAgent 是一个可运行、可观察、可扩展的 Agent Harness 教学与实验工程，以具备任务规划、工具调用和沙箱执行能力的 AI Agent 应用为实践载体。用户通过 Web 界面发起任务，Agent 将目标拆解为步骤，在执行过程中调用工具、更新计划，并将进度和结果实时返回界面。

这里的可运行指能实际执行任务，可观察指能查看计划、事件与工具结果，可扩展指可通过工具层和 MCP/A2A 接入能力；这些能力的具体范围与限制在课程中结合实现讨论。

课程围绕模型之外的运行机制展开：如何组织 Context、连接工具与执行环境、控制任务、保存状态，以及观察和验证结果。lessons 建立通用概念与设计理解，labs 隔离关键机制，现有应用用于研究这些机制的组合与取舍。学习入口见 [课程目录](lessons/README.md)。
