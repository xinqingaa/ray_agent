...
本文描述当前系统的模块边界、执行与数据流，以及理解实现时需要注意的限制。用户在页面上的操作见 [项目首页](../README.md)；发送之后请求怎样走完见下文「一次任务的执行」。内容依据代码核对，完整运行验证的阶段状态见 [执行计划](../PLAN.md)。
...
![用户请求进入规划与执行，在环境中落地，进度回到页面，选定文件成为可下载附件](assets/task-after-send.svg)
