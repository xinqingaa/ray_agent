...
![RayAgent 架构：应用协调连接任务运行，模型、沙箱和外部服务执行动作，消息与文件分别保存](assets/architecture-overview.png)

图中蓝线表示请求、调用与返回，绿线表示事件和数据传递。中间区域属于 API 进程，外部服务和存储通过适配访问；双向箭头表示交互关系，不表示各操作在同一事务中完成。
...
![任务运行模型：外层选择并更新步骤，内层循环调用工具，等待用户输入后可以继续](assets/task-lifecycle.png)
