...
| Redis | 任务输入输出流与事件传递 |
| 文件存储 | 附件、执行文件与截图；由 `FILE_STORAGE_BACKEND` 选择本地磁盘或腾讯云 COS |
...
| 会话状态 | `pending` / `running` / `waiting` / `completed` / `failed`。失败后同一会话可再发消息重新规划，不要求新开任务 |
...
文件内容由 `FILE_STORAGE_BACKEND` 决定保存在本地磁盘或腾讯云 COS，元数据与会话关系由应用和数据库管理。上传附件、沙箱文件与浏览器截图跨越不同资源，排查时应确认失败发生在哪一步。页面访问地址由对应存储实现提供，不在执行流程中写死 COS 域名。
